...
public class ProcessMemory
{
    ...
    public ProcessMemory(Process proc, IntPtr handle, bool trust = false)
    {
        mainProcess = new Process[] { proc };
        processHandle = OpenProcess(0x001F0FFF, false, (int)handle);
        _opened = true;
        processName = "lithtech";
        processModule = mainProcess[0].MainModule;
    }

    public int GetModuleAddress(string moduleName)
    {
        foreach (ProcessModule module in mainProcess[0].Modules)
        {
    ...
    {
        return mainProcess[0].StartTime;
    }
    public int GetProcessID()
    {
        return mainProcess[0].Id;
    }
    ...
    public bool Traverse(IntPtr addr, IEnumerable<int> offsets, out IntPtr result)
    {
        ...
        {
            result = (IntPtr)ReadInt32(result);
    ...
    public string TraverseStringASCII(IntPtr addr, IEnumerable<int> offsets, uint size) => Traverse(addr, offsets, out IntPtr result)
        ...
        : null;
    public string TraverseStringASCII(IntPtr addr, IEnumerable<int> offsets, int final, uint size) => Traverse(addr, offsets, out IntPtr result)
        ? ReadStringASCII(result + final, size)
        : null;
    ...
    public int? TraverseInt32(IntPtr addr, IEnumerable<int> offsets, int final = 0) => Traverse(addr, offsets, out IntPtr result)
        ? ReadInt32(result + final)
        : (int?)null;
    ...
    public uint? TraverseUInt32(IntPtr addr, IEnumerable<int> offsets, int final = 0) => Traverse(addr, offsets, out IntPtr result)
        ? ReadUInt32(result + final)
        : (uint?)null;
    ...
    public float? TraverseFloat(IntPtr addr, IEnumerable<int> offsets, int final = 0) => Traverse(addr, offsets, out IntPtr result)
        ? ReadFloat(result + final)
        : (float?)null;
